...
for %%e in (%libraries%) do (
    if exist "%EXTERNALS_DIR%\%%e" (
        ...
        git clone --depth 1 https://github.com/%ORG%/spython-source-deps --branch %%e "%EXTERNALS_DIR%\%%e"
    ) else (
        echo.Fetching %%e...
...
for %%b in (%binaries%) do (
    if exist "%EXTERNALS_DIR%\%%b" (
        ...
        git clone --depth 1 https://github.com/%ORG%/spython-bin-deps --branch %%b "%EXTERNALS_DIR%\%%b"
    ) else (
        echo.Fetching %%b...
