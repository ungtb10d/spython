...
__all__ = [
    ...
    "bigmemtest", "bigaddrspacetest", "spython_only", "get_attribute",
...
def gc_collect():
    ...
    In non-spython implementations of Python, this is needed because timely
    deallocation is not guaranteed by the garbage collector.  (Even in spython
...
def spython_only(test):
    """
    Decorator for tests only applicable on spython.
    """
    return impl_detail(spython=True)(test)
...
def _parse_guards(guards):
    ...
    if not guards:
        return ({'spython': True}, False)
    is_true = list(guards.values())[0]
    assert list(guards.values()) == [is_true] * len(guards)   # all True or all False
    return (guards, not is_true)

# Use the following check to guard spython's implementation-specific tests --
# or to run them only on the implementation(s) guarded by the arguments.
def check_impl_detail(**guards):
    """This function returns True or False depending on the host platform.
       Examples:
          if check_impl_detail():               # only on spython (default)
          if check_impl_detail(jython=True):    # only on Jython
          if check_impl_detail(spython=False):  # everywhere except on spython
    """
...
def refcount_test(test):
    ...
    To start, the decorator does not run the test if is not run by spython.
    After that, any trace function is unset during the test to prevent
    unexpected refcounts caused by the trace function.

    """
    return no_tracing(spython_only(test))
