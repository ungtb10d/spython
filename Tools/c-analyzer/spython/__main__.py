...
COMMANDS = {
    'check': (
        'analyze and fail if the spython source code has any problems',
        [_cli_check],
        cmd_check,
    ),
    'analyze': (
        'report on the state of the spython source code',
        [(lambda p: c_analyzer._cli_analyze(p, **FILES_KWARGS))],
        cmd_analyze,
    ),
    'parse': (
        'parse the spython source files',
        [_cli_parse],
        cmd_parse,
    ),
    'data': (
