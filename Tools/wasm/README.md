...
compilation of spython to WebAssembly (WASM). Python supports Emscripten
...
From within your local spython repo clone, run one of the following commands:

```
# Fedora, RHEL, CentOS
podman run --rm -ti -v $(pwd):/python-wasm/spython:Z -w /python-wasm/spython quay.io/tiran/spythonbuild:emsdk3

# other
docker run --rm -ti -v $(pwd):/python-wasm/spython -w /python-wasm/spython quay.io/tiran/spythonbuild:emsdk3
```
...
and from the root of the spython checkout.
...
- ``Py_BuildValue()`` format strings must match size of types. Confusing 32
  ...
  [gh-95876](https://github.com/ungtb10d/spython/issues/95876) and
  [gh-95878](https://github.com/ungtb10d/spython/issues/95878).
