...
Consolidate spython's global runtime state under a single struct.  This
...
Compiling regular expression in debug mode on spython now displays the
...
zlib is no longer bundled in the spython source, instead it is downloaded on
...
spython has provided the own implementation for thread-local storage (TLS)
