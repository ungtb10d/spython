...
pymain_run_command(wchar_t *command)
{
    ...
    if (PySys_Audit("spython.run_command", "O", unicode) < 0) {
...
pymain_run_module(const wchar_t *modname, int set_argv0)
{
    ...
    if (PySys_Audit("spython.run_module", "u", modname) < 0) {
        return pymain_exit_err_print();
    }
...
{
    if (PySys_Audit("spython.run_file", "O", filename) < 0) {
        return pymain_exit_err_print();
    }
...
#endif
    if (PySys_Audit("spython.run_startup", "O", startup) < 0) {
        goto error;
    }
...
pymain_run_interactive_hook(int *exitcode)
{
    ...
    if (PySys_Audit("spython.run_interactivehook", "O", hook) < 0) {
        goto error;
    }
...
pymain_run_stdin(PyConfig *config)
{
    ...
    if (PySys_Audit("spython.run_stdin", NULL) < 0) {
        return pymain_exit_err_print();
    }
