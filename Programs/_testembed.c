...
static int _audit_hook(const char *event, PyObject *args, void *userdata)
{
    ...
    if (strcmp(event, "_testembed.raise") == 0) {
        ...
        return 0;
    } else if (strcmp(event, "spython._PySys_ClearAuditHooks") == 0) {
...
{
    printf("%s\n", event);
    if (strcmp(event, "spython.PyInterpreterState_New") == 0) {
...
{
    AuditRunCommandTest test = {"spython.run_command"};
...
{
    AuditRunCommandTest test = {"spython.run_file"};
...
{
    AuditRunCommandTest test = {"spython.run_interactivehook", 10};
...
{
    AuditRunCommandTest test = {"spython.run_startup", 10};
    wchar_t *argv[] = {PROGRAM_NAME};
    return run_audit_run_test(Py_ARRAY_LENGTH(argv), argv, &test);
}
...
{
    AuditRunCommandTest test = {"spython.run_stdin"};
    wchar_t *argv[] = {PROGRAM_NAME};
    return run_audit_run_test(Py_ARRAY_LENGTH(argv), argv, &test);
}
