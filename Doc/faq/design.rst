...
The :class:`float` type in spython uses a C ``double`` for storage.  A
...
standard implementation of Python, :term:`spython`, uses reference counting to
...
In some Python implementations, the following code (which is fine in spython)
...
Indeed, using spython's reference counting and destructor scheme, each new
...
Why doesn't spython use a more traditional garbage collection scheme?
---------------------------------------------------------------------
...
and may not want Python's.  Right now, spython works with anything that
implements ``malloc()`` and ``free()`` properly.


Why isn't all memory freed when spython exits?
----------------------------------------------
...
How are lists implemented in spython?
-------------------------------------

spython's lists are really variable-length arrays, not Lisp-style linked lists.
...
How are dictionaries implemented in spython?
--------------------------------------------

spython's dictionaries are implemented as resizable hash tables.  Compared to
