...
===============================================
Instrumenting spython with DTrace and SystemTap
===============================================
...
As of Python 3.6, spython can be built with embedded "markers", also
known as "probes", that can be observed by a DTrace or SystemTap script,
making it easier to monitor what the spython processes on a system are
doing.

.. impl-detail::

   DTrace markers are implementation details of the spython interpreter.
   No guarantees are made about probe compatibility between versions of
   spython. DTrace scripts can stop working or work incorrectly without
   warning when changing spython versions.
...
build spython with the embedded markers for SystemTap, the SystemTap
...
spython must then be :option:`configured with the --with-dtrace option
<--with-dtrace>`:
...
For a :option:`--enable-shared` build of spython, the markers are contained within the
...
(assuming a :ref:`debug build <debug-build>` of spython 3.6)
...
Here is a tapset file, based on a non-shared build of spython:
...
running spython code, showing the top 20 most frequently entered bytecode
