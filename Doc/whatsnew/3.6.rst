...
PEP 523: Adding a frame evaluation API to spython
-------------------------------------------------
...
* Cross-compiling spython with the Android NDK and the Android API level set to
...
* ``inspect.getmoduleinfo()`` was removed (was deprecated since spython 3.3).
...
* The undocumented ``IN``, ``CDROM``, ``DLFCN``, ``TYPES``, ``CDIO``, and
  ...
  <https://github.com/ungtb10d/spython/blob/v3.6.15/Tools/scripts/h2py.py>`_.
