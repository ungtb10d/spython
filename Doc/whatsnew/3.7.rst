...
When in UTF-8 mode, spython ignores the locale settings, and uses the
...
API <thread-specific-storage-api>` to spython which supersedes use of the
existing TLS API within the spython interpreter, while deprecating the existing
...
the underlying TLS implementation.  Therefore, this will allow to build spython
...
.. seealso::
    ...
    :pep:`539` -- A New C-API for Thread-Local Storage in spython
...
changes to the core spython interpreter. Now type hints and the :mod:`typing`
...
* warnings enabled via command line options (including those for :option:`-b`
  and the new spython-specific :option:`-X` ``dev`` option) are always passed
...
* warnings filters enabled via the command line or the environment now have the
  following order of precedence:
     ...
     * any filters specified with the :envvar:`PYTHONWARNINGS` environment
       variable
     * any other spython specific filters (e.g. the ``default`` filter added
       for the new ``-X dev`` mode)
     * any implicit filters defined directly by the warnings machinery

* in :ref:`spython debug builds <debug-build>`, all warnings are now displayed
...
Other spython Implementation Changes
====================================
...
* OpenSSL 0.9.8 and 1.0.1 are no longer supported, which means building spython
  ...
  spython's own `CI configuration file
  <https://github.com/ungtb10d/spython/blob/v3.7.13/.travis.yml>`_ provides an
  example of using the SSL
  :source:`compatibility testing infrastructure <Tools/ssl/multissltests.py>` in
  spython's test suite to build and link against OpenSSL 1.1.0 rather than an
...
In preparation for potential future changes to the public spython runtime
initialization API (see :pep:`432` for an initial, but somewhat outdated,
draft), spython's internal startup
...
applications and users of the regular spython CLI, they're being mentioned
...
either in embedding applications, or in spython itself.
