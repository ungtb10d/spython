...
* SSLv3 is now disabled throughout the standard library.
  ...
  backported to spython 3.4 and 2.7.)
...
.. note::
   ...
   .. versionchanged:: 3.5.2
      Starting with spython 3.5.2, ``__aiter__`` can directly return
...
some platforms (the tag is optional, and spython will import extensions without
it, although if the tag is present and mismatched, the extension won't be
loaded):

* On Linux, extension module filenames end with
  ``.spython-<major><minor>m-<architecture>-<os>.pyd``:
...
Windows XP is no longer supported by Microsoft, thus, per :PEP:`11`, spython
...
To simplify cross-compilation, and to ensure that spython can reliably be
