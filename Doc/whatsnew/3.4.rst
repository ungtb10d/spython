...
spython implementation improvements:
...
other smaller improvements, spython optimizations, deprecations, and potential
porting issues.
...
For spython :ref:`source builds on POSIX systems <building-python-on-unix>`,
...
On Windows and Mac OS X, the spython installers now default to installing
``pip`` along with spython itself (users may opt out of installing it
...
release candidate of the release of spython with which it ships (this applies
...
stdlib; in spython it is overridden by the C accelerated version, but it is
...
a formally supported interface.  It has always existed in spython's
...
:func:`~inspect.signature` now supports duck types of spython functions,
...
blocks allocated by the interpreter.  (In spython with the default
...
spython Implementation Changes
==============================


.. _whatsnew-pep-445:

PEP 445: Customization of spython Memory Allocators
---------------------------------------------------

:pep:`445` adds new C level interfaces to customize memory allocation in
the spython interpreter.
...
in spython. With it, objects with :meth:`__del__` methods, as well as
...
current round of improvements.)  The PEP unifies spython's hash code to make it
...
the PEP does not alter any existing spython APIs.
...
"Argument Clinic" (:pep:`436`) is now part of the spython build process
...
* The new :c:func:`Py_SetStandardStreamEncoding` pre-initialization API
  allows applications embedding the spython interpreter to reliably force
...
* The spython source can now be compiled using the address sanity checking
...
* The spython Windows installer now adds ``.py`` to the :envvar:`PATHEXT`
...
* The [X refs, Y blocks] output of a debug (``--with-pydebug``) build of the
  spython interpreter is now off by default.  It can be re-enabled using the
...
* Strings between ``from __future__ import ...`` statements now *always* raise
  ...
  interstitial string would sometimes be ignored.  This brings spython into
