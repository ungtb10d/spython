...
* Python 3.11 is between 10-60% faster than Python 3.10.
  ...
  See :ref:`whatsnew311-faster-spython` for details.
...
:ref:`whatsnew311-faster-spython` project, which is covered in its own section.
...
.. _whatsnew311-faster-spython:

Faster spython
==============

spython 3.11 is on average `25% faster <https://github.com/faster-spython/ideas#published-results>`_
than spython 3.10 when measured with the
...
In 3.11, when spython detects Python code calling another Python function,
...
:pep:`659` is one of the key parts of the faster spython project. The general
...
.. _whatsnew311-faster-spython-misc:
...
.. _whatsnew311-faster-spython-faq:
...
| A: You don't have to change your code. Write Pythonic code that follows common
  best practices. The Faster spython project optimizes for common code
  patterns we observe.
|
|
| Q: Will spython 3.11 use more memory?
|
...
.. _whatsnew311-faster-spython-about:

About
-----

Faster spython explores optimizations for :term:`spython`. The main team is
...
* spython now has :pep:`11` :pep:`Tier 3 support <11#tier-3>` for
...
* spython can now be built with the
...
* spython will now use 30-bit digits by default for the Python :class:`int`
...
* The non-limited API files ``cellobject.h``, ``classobject.h``, ``code.h``, ``context.h``,
  ...
  the ``Include/spython`` directory. Moreover, the ``eval.h`` header file was
