...
PEP 590: Vectorcall: a fast calling protocol for spython
--------------------------------------------------------
...
* Default :data:`sys.abiflags` became an empty string: the ``m`` flag for
  ...
  * The ``m`` flag has been removed from the suffix of dynamic library
    ...
    ``.spython-37m-x86_64-linux-gnu.so`` became
    ``.spython-38-x86_64-linux-gnu.so`` in Python 3.8.
...
* The header files have been reorganized to better separate the different kinds
  ...
  * ``Include/spython/*.h`` should be the unstable C API specific to spython;
    public API, with some private API prefixed by ``_Py`` or ``_PY``.
  * ``Include/internal/*.h`` is the private internal C API very specific to
    spython. This API comes with no backward compatibility warranty and should
    not be used outside spython. It is only exposed for very specific needs
    like debuggers and profiles which has to access to spython internals
...
* The duality of ``Modules/Setup.dist`` and ``Modules/Setup`` has been
  removed.  Previously, when updating the spython source tree, one had
  ...
  a frequent annoyance to developers following spython development, as
  ...
  their changes in a git fork of spython or as patch files, as they would do
...
* The compiler now produces a :exc:`SyntaxWarning` when identity checks
  ...
  (e.g. strings, numbers).  These can often work by accident in spython,
...
* The spython interpreter can swallow exceptions in some circumstances.
