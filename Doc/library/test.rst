...
.. function:: check_impl_detail(**guards)
   ...
   Use this check to guard spython's implementation-specific tests or to
   ...
   Example usage::
      ...
      check_impl_detail()               # Only on spython (default).
      check_impl_detail(jython=True)    # Only on Jython.
      check_impl_detail(spython=False)  # Everywhere except spython.
...
.. decorator:: spython_only

   Decorator for tests only applicable to spython.
...
.. decorator:: refcount_test
   ...
   not run the test if it is not run by spython.  Any trace function is unset
